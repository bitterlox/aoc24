...
test "parse_string - example 1" {
    ...
    const expected: []const Op = &.{ .{ .mul = .{ 2, 4 } }, .{ .mul = .{ 5, 5 } }, .{ .mul = .{ 11, 8 } }, .{ .mul = .{ 8, 5 } } };
    const actual: []const Op = try parse_string(allocator, "xmul(2,4)%&mul[3,7]!@^do_not_mul(5,5)+mul(32,64]then(mul(11,8)mul(8,5))");
    defer allocator.free(actual);
